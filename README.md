...
```bash
haomingSAM$ sam build --use-container
```
...
```bash
haomingSAM$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
haomingSAM$ sam logs -n HelloWorldFunction --stack-name haomingSAM --tail
```
...
```bash
haomingSAM$ pip install -r tests/requirements.txt --user
# unit test
haomingSAM$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
haomingSAM$ AWS_SAM_STACK_NAME=<stack-name> python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name haomingSAM
```
